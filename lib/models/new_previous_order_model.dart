class NewPrevousOrderModel {
  int? status;
  String? message;
  List<PreviousOrder>? data;

  NewPrevousOrderModel({this.status, this.message, this.data});

  NewPrevousOrderModel.fromJson(Map<String, dynamic> json) {
    status = json['status'];
    message = json['message'];
    if (json['data'] != null) {
      data = <PreviousOrder>[];
      json['data'].forEach((v) {
        data!.add(new PreviousOrder.fromJson(v));
      });
    }
  }
